import { gameService } from '../services/game.service.js';

export const gameController = {
  async getAll(req, res, next) {
    try {
      const data = await gameService.getAllGames(req.query);
      res.status(200).json({ ok: true, status: 200, data });
    } catch (error) {
      next(error);
    }
  },

  async getById(req, res, next) {
    try {
      const data = await gameService.getGameById(req.params.id);
      res.status(200).json({ ok: true, status: 200, data });
    } catch (error) {
      next(error);
    }
  },

  async create(req, res, next) {
    try {
      const data = await gameService.createGame(req.body);
      res.status(201).json({ ok: true, status: 201, data });
    } catch (error) {
      next(error);
    }
  },

  async update(req, res, next) {
    try {
      const data = await gameService.updateGame(req.params.id, req.body);
      res.status(200).json({ ok: true, status: 200, data });
    } catch (error) {
      next(error);
    }
  },

  async delete(req, res, next) {
    try {
      const data = await gameService.deleteGame(req.params.id);
      res.status(200).json({ ok: true, status: 200, message: data.message });
    } catch (error) {
      next(error);
    }
  }
};
